...
class WordSequence(nn.Module):
    def __init__(self, data):
        ...
        self.domain_task_id = data.task_alphabet.get_index(data.domain_task_name)
    ...
    def forward(self, mode, word_inputs, word_seq_lengths):
        ...
        if mode == 'ner1':
            ...
            task_idx = self.domain_task_id


        lstm_param = self.LSTM_param_generator(task_idx)
        ...
        if 'lm' in mode:
            ...
            outputs_forward, outputs_backward = outputs.chunk(2, -1)
            outputs = torch.cat((outputs_forward, outputs_backward), -1)
            outputs = ReverseLayerF.apply(outputs, 10)
