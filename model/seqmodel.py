...
class SeqModel(nn.Module):
    def __init__(self, data):
        ...
        self.hidden2tag_domain = nn.Linear(data.HP_hidden_dim, data.label_alphabet_domain_size)
        ...
        if self.gpu:
            ...
            self.hidden2tag_domain = self.hidden2tag_domain.cuda()
    ...
    def neg_log_likelihood_loss(self, mode, outs, batch_size, seq_len, batch_label, mask):
        if mode == 'ner1':
            ...
            outs = self.hidden2tag_domain(outs)
            loss_function = nn.NLLLoss(ignore_index=0, size_average=False)
            outs = outs.view(batch_size * seq_len, -1)
            score = F.log_softmax(outs, 1)
            total_loss = loss_function(score, batch_label.view(batch_size * seq_len))
