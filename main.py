    ...
    for index in range(len(gold_results)):    
        ...
        if index == len(gold_results[index])-1:
            ...
            if isNeu and not isPos and not isNeg:
                ...
                neu = {}
                isNeu = False
    return pos_pred_word, neg_pred_word, neu_pred_word, \
            pos_true_word, neg_true_word, neu_true_word
...
def train(train_data):
    ...
    lambda_weight = [random.uniform(0, 1), random.uniform(0, 1), random.uniform(0, 1), \
                    ...
                    if DWA else [0.8, 1, 0.5, 0.5, 1, 1, 1]
    ...
    for idx in range(train_data.HP_iteration):
        ...
        random.shuffle(train_data.domain_train_idx)
        ...
        domain_batch_size = len(train_data.domain_train_idx) // batch_nums
        ...
        if DWA:
            ...
            if  idx > 1:
                ...
                SUM = np.exp(w_0 / T) + np.exp(w_2 / T) + np.exp(w_4 / T) + np.exp(w_1 / T) + np.exp(w_3 / T) + np.exp(w_5 / T) + np.exp(w_6 / T)

                lambda_weight[0] = 7*np.exp(w_0 / T) / SUM
                lambda_weight[1] = 7*np.exp(w_1 / T) / SUM
                lambda_weight[2] = 7*np.exp(w_2 / T) / SUM
                lambda_weight[3] = 7*np.exp(w_3 / T) / SUM
                lambda_weight[4] = 7*np.exp(w_4 / T) / SUM
                lambda_weight[5] = 7*np.exp(w_5 / T) / SUM
                lambda_weight[6] = 7*np.exp(w_6 / T) / SUM
        ...
        for batch_id in range(batch_nums):
            ...
            domain_data = train_data.domain_train_idx[batch_id * domain_batch_size: (batch_id + 1) * domain_batch_size \
            if(batch_id + 1) * domain_batch_size < len(train_data.domain_train_idx) else len(train_data.domain_train_idx)]
            ...
            domain_batch_data = batchify_with_label(domain_data, train_data.HP_gpu)
            
            model_loss = 0
            total_weight = 0

            for i in lambda_weight:
                total_weight += i
            ...
            if idx / 2 == 0:
                ...
                loss, perplexity, tag_seq_forward, tag_seq_backward, tag_seq = \
                     model.loss('domain', domain_batch_data[0], domain_batch_data[1],  domain_batch_data[6], domain_batch_data[7], domain_batch_data[8])
                lossA.append(loss)  
        ...
        logger.info('lambda_weight: %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f'%(
            lambda_weight[0],lambda_weight[1],lambda_weight[2],lambda_weight[3],lambda_weight[4],lambda_weight[5],lambda_weight[6]))
    ...
    logger.info("the best Appliance SA test score is in epoch %s, test:%.4f" % (best_epoch_Product, SA_Pro[best_epoch_Product]))
