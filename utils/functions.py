...
def read_domain_instance(input_file_train1, input_file_train2, input_file_test2, domain_alphabet, word_alphabet, number_normalized, max_sent_length):
    instance_train_text, instance_train_idx = [], []
    words, labels = [],[]
    word_idx, label_idx = [], []
    src = [input_file_train1]
    for input_file in src:
        ...
        for line in open(input_file, 'r').readlines():
            ...
            for word in pairs:
                ...
                labels.append(LABEL)
                label_idx.append(domain_alphabet.get_index(LABEL))
               
            if len(words) > 0 and len(words) < max_sent_length:
                instance_train_text.append([words, labels])
                instance_train_idx.append([word_idx, label_idx])

            words, labels = [], []
            word_idx, label_idx = [], []

    instance_test_text, instance_test_idx = [], []
    words, labels = [], []
    word_idx, label_idx = [], []       
    tgt = [input_file_train2, input_file_test2]
    for input_file in tgt:
        LABEL = 'target'
        for line in open(input_file, 'r').readlines():
            if len(line) > 2:
                pairs = line.strip().split(' ')
            ...
            if len(words) > 0 and len(words) < max_sent_length:
                instance_test_text.append([words, labels])
                instance_test_idx.append([word_idx, label_idx])

            words, labels = [], []
            word_idx, label_idx = [], []
    return instance_train_text, instance_train_idx, instance_test_text, instance_test_idx
